...
namespace AssemblyCSharp.Assets.Code.Features.SpeedDuel
{
    public class PlaymatViewLogic : MonoBehaviour
    {
        ...
        {
            var mappedScale = MapToNewSliderValueToNewValue(scale, _scaleSlider.minValue, _scaleSlider.maxValue, 0.1f, 5f);
            _playmatShell.transform.localScale = new Vector3(mappedScale, mappedScale, mappedScale);
        }

        private float MapToNewSliderValueToNewValue(float value,
                                                    float originalMin,
                                                    float originalMax,
                                                    float newMin,
                                                    float newMax)
