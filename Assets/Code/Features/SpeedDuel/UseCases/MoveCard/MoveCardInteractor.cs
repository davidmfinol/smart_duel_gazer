...
namespace Code.Features.SpeedDuel.UseCases.MoveCard
{
    ...
    {
        PlayerState Execute(PlayerState playerState, PlayCard card, CardPosition position, Zone newZone,
    ...
    public class MoveCardInteractor : IMoveCardInteractor
    {
        ...
        public PlayerState Execute(PlayerState playerState, PlayCard card, CardPosition position, Zone newZone,
            GameObject speedDuelField)
        {
            ...
            var updatedZones = newZone.ZoneType == oldZone.ZoneType
                ? _updateCardPositionUseCase.Execute(playerState, card, position, playerZones, oldZone, speedDuelField)
                : _moveCardToNewZoneUseCase.Execute(playerState, card, position, playerZones, oldZone, newZone, speedDuelField);
